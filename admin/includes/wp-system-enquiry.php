...
if ( isset( $_POST['wpsl-submit-enquiry'] ) && isset( $_POST['wpsl-send-enquiry-nonce'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['wpsl-send-enquiry-nonce'] ) ), 'wpsl-enquiry' ) ) {
	$email = isset( $_POST['wpsl-support-agent-email'] ) ? sanitize_text_field( wp_unslash( $_POST['wpsl-support-agent-email'] ) ) : '';
	...
	if ( ! filter_var( $email, FILTER_VALIDATE_EMAIL ) ) {
		...
		global $wpdb;
		$message = isset( $_POST['wpsl-enquiry-message'] ) ? sanitize_text_field( wp_unslash( $_POST['wpsl-enquiry-message'] ) ) : '';
		...
		<html>
			...
			<body>
				...
				<table cellspacing="0" style="border: 2px dashed #FB4314; width: 100%; height: 90px;">
					...
					<tr>
						<th style="padding: 10px;">Current IP:</th><td>' . isset( $_SERVER['REMOTE_ADDR'] ) ? sanitize_text_field( wp_unslash( $_SERVER['REMOTE_ADDR'] ) ) : '' . '</td>
					</tr>
					<tr style="background-color: #e0e0e0;">
		...
		<form action="" method="POST">
			<table class="form-table">
				<tbody>
					<!-- SUPPORT AGENT EMAIL -->
					<tr>
						<th scope="row"><label for="support-agent-email"><?php esc_html_e( 'Support Agent Email', 'wp-system-log' ); ?></label></th>
